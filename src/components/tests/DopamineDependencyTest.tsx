...
const RESULTS: Record<Level, Record<Locale, { title: string; color: string; bg: string; symptoms: string[]; tips: string[] }>> = {
  ...
  critical: {
    ...
    zh: { title: '嚴重依賴 ⚠️', color: 'text-green-700', bg: 'bg-green-50 border-green-200', symptoms: ['沒有即時刺激就難以正常運作', '限制數字使用時出現嚴重戒斷症狀', '基本生活節奏（睡眠、飲食）被打亂', '優先考慮數字關係而非現實關係'], tips: ['建議諮詢行為成癮專科醫生', '嘗試一週完全數字排毒', '積極利用身邊人的支持', '檢查潛在的焦慮或抑鬱症'] },
  },
}
