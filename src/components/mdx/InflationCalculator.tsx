...
export default function InflationCalculator() {
  ...
  return (
    <div className="my-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <h3 className="mb-6 text-xl font-bold text-gray-900 dark:text-white">인플레이션 구매력 계산기</h3>

      <div className="mb-4 flex gap-3">
        <button
          onClick={() => setMode('future')}
          className={`flex-1 rounded-lg py-2 text-sm font-medium transition ${mode === 'future' ? 'bg-red-600 text-white' : 'border border-gray-300 text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400'}`}
        >
          현재 돈의 미래 가치
        </button>
        <button
          onClick={() => setMode('today')}
          className={`flex-1 rounded-lg py-2 text-sm font-medium transition ${mode === 'today' ? 'bg-red-600 text-white' : 'border border-gray-300 text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400'}`}
        >
          미래에 필요한 금액
        </button>
      </div>
        ...
        <div className="mt-6 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-3">
            <div className="rounded-xl bg-gray-50 p-4 dark:bg-gray-700">
              ...
              <p className="text-base font-bold text-gray-700 dark:text-gray-300">{formatKRW(parseFloat(amount.replace(/,/g, '')))}원</p>
            </div>
            <div className="rounded-xl bg-red-50 p-4 dark:bg-red-900/20">
              <p className="text-xs text-gray-500">
                {mode === 'future' ? `${years}년 후 구매력` : `${years}년 후 필요 금액`}
              </p>
              <p className="text-base font-bold text-red-600 dark:text-red-400">{formatKRW(result.adjusted)}원</p>
            </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-600">
            <table className="w-full text-xs">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  <th className="px-3 py-2 text-left font-semibold text-gray-700 dark:text-gray-300">년도</th>
                  <th className="px-3 py-2 text-left font-semibold text-gray-700 dark:text-gray-300">
                    {mode === 'future' ? '실질 구매력' : '필요 금액'}
                  </th>
                  <th className="px-3 py-2 text-left font-semibold text-gray-700 dark:text-gray-300">변화율</th>
                </tr>
              </thead>
              <tbody>
                {displayRows.map((row) => {
                  ...
                  return (
                    <tr key={row.year} className="border-t border-gray-100 dark:border-gray-700">
                      <td className="px-3 py-2 font-medium text-gray-700 dark:text-gray-300">{row.year}년 후</td>
                      <td className="px-3 py-2 text-gray-900 dark:text-white">{formatKRW(val)}원</td>
