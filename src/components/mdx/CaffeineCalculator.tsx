...
function getSensitivityInfo(weight: number, total: number) {
  ...
  if (perKg < 2) return { level: '낮음', color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/20', desc: '안전 범위입니다. 카페인 부작용 위험이 낮습니다.' };
  if (perKg < 4) return { level: '보통', color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20', desc: '일반 성인 적정 범위입니다.' };
  if (perKg < 6) return { level: '주의', color: 'text-yellow-600', bg: 'bg-yellow-50 dark:bg-yellow-900/20', desc: 'FDA 권장 한도에 근접합니다. 불안·심박 증가에 주의하세요.' };
  return { level: '위험', color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-900/20', desc: '권장 한도(6mg/kg)를 초과합니다. 두통·심계항진·불면 위험이 높습니다.' };
}
...
export default function CaffeineCalculator() {
  ...
  return (
    <div className="my-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <h3 className="mb-6 text-xl font-bold text-gray-900 dark:text-white">카페인 섭취량 계산기</h3>
      ...
      <p className="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">오늘 섭취한 음식/음료 수량 입력</p>

      <div className="mb-4 space-y-2">
        {DRINKS.map((d) => (
          <div key={d.name} className="flex items-center gap-3">
            <span className="w-52 text-sm text-gray-600 dark:text-gray-400">
              {d.name} <span className="text-xs text-gray-400">({d.caffeine}mg)</span>
            </span>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setQty(d.name, (quantities[d.name] || 0) - 1)}
                className="h-7 w-7 rounded-full border border-gray-300 text-sm hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700"
              >−</button>
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            <div className="rounded-xl bg-gray-50 p-4 dark:bg-gray-700">
              ...
              <p className="text-lg font-bold text-gray-700 dark:text-gray-300">400mg</p>
              <p className="text-xs text-gray-400">잔여: {Math.max(0, 400 - result.total)}mg</p>
            </div>
            <div className="rounded-xl bg-gray-50 p-4 dark:bg-gray-700">
              <p className="text-xs text-gray-500">취침 시 잔존량 (16h 후)</p>
              <p className="text-lg font-bold text-gray-700 dark:text-gray-300">{bedtimeRemainder}mg</p>
          ...
          {result.breakdown.length > 0 && (
            <div className="space-y-1">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">섭취 내역</p>
              {result.breakdown.map((b, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">{b.name}</span>
                  <span className="font-medium text-gray-900 dark:text-white">{b.mg}mg</span>
                </div>
              ))}
            </div>
          )}
